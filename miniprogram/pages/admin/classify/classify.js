// pages/admin/classify/classify.js
Page({
 ...
 */
  ...
  */
  ...
  formClassifySubmit: async function (e) {
    ...
    if (classifyName === undefined || classifyName === "") {
      ...
      let res = await wx.a.addBaseClassify(classifyName, classifyDesc)
      console.info(res)
      wx.hideLoading()
      if (res.result) {
        that.setData({
  ...
  deleteClassifyById: async function (e) {
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          wx.a.deleteConfigById(classifyId).then(res => {
  ...
  */
  ...
  savePostsRelatedClassify: async function (e) {
    ...
    try {
      let res = await wx.a.updateBatchPostsClassify(that.data.curClassifyName, that.data.tabCur == 1 ? "add" : "delete", posts)
      console.info(res)
      if (res.result) {
        wx.showToast({
          title: '处理成功',
