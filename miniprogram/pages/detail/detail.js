...
Page({
  ...
  onCreatePoster: async function () {
    ...
    posterwx.c.blocks = blocks;//海报内图片的外框
    posterwx.c.texts = texts; //海报的文字
    posterwx.c.images = images;
  ...
  */
  ...
  getDetail: async function (blogId) {
    ...
    let that = this
    let postDetail = await wx.a.getPostDetail(blogId);
  ...
  getPostRelated: async function (blogId) {
    ...
    console.log('getPostRelated', where)
    let postRelated = await wx.a.getPostRelated(where, 1);
    console.log('postRelated', postRelated)
    let that = this;
    for (var item of postRelated.data) {
      if (wx.c.postRelatedType.COLLECTION === item.type) {
        that.setData({
          collection: { status: true, text: "已收藏", icon: "favorfill" }
        })
        continue;
      }
      if (wx.c.postRelatedType.ZAN === item.type) {
        that.setData({
  ...
  onReachBottom: async function () {
    ...
    try {
      ...
      let commentList = await wx.a.getPostComments(page, that.data.post._id)
  ...
  postCollection: async function () {
    ...
    try {
      ...
      if (collection.status === true) {
        let result = await wx.a.deletePostCollectionOrZan(that.data.post._id, wx.c.postRelatedType.COLLECTION)
        console.info(result)
        that.setData({
          collection: { status: false, text: "收藏", icon: "favor" }
        })
        ...
        let data = {
          ...
          type: wx.c.postRelatedType.COLLECTION
        }
        await wx.a.addPostCollection(data)
        that.setData({
          collection: { status: true, text: "已收藏", icon: "favorfill" }
        })
  ...
  postZan: async function () {
    ...
    try {
      ...
      if (zan.status === true) {
        let result = await wx.a.deletePostCollectionOrZan(that.data.post._id, wx.c.postRelatedType.ZAN)
        console.info(result)
        that.setData({
          zan: { status: false, text: "点赞", icon: "appreciate" }
        })
        ...
        let data = {
          ...
          postUrl: that.data.post.defaultImageUrl,
          postDigest: that.data.post.digest,
          type: wx.c.postRelatedType.ZAN
        }
        await wx.a.addPostZan(data)
        that.setData({
          zan: { status: true, text: "已赞", icon: "appreciatefill" }
        })
      ...
      success(res) {
        if (res.confirm) {
          wx.requestSubscribeMessage({
            tmplIds: [wx.c.subcributeTemplateId],
            success(res) {
              console.info('requestSubscribeMessage', res)
              console.info(res[wx.c.subcributeTemplateId])
              that.submitContent(content, commentPage, res[wx.c.subcributeTemplateId]).then((res) => { console.info(res) })
            },
            fail(res) {
              console.info(res)
              wx.showToast({
  ...
  submitContent: async function (content, commentPage, accept) {
    ...
    try {
      ...
      if (that.data.commentId === "") {
        ...
        await wx.a.addPostComment(data, accept)
      }
      else {
        var childData = [{
          cOpenId: app.globalData.openid,
        ...
        await wx.a.addPostChildComment(that.data.commentId, that.data.post._id, childData, accept)
      }

      let commentList = await wx.a.getPostComments(commentPage, that.data.post._id)
      if (commentList.data.length === 0) {
        that.setData({
          nomore: true
        })
